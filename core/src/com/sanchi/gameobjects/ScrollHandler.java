...
public class ScrollHandler {
	...
	// Return true if ANY pipe hits Zoi.
	public boolean collides(Zoi zoi) {
		if (!pipe1.isScored()
				&& pipe1.getX() + (pipe1.getWidth() / 2) < zoi.getX()
						+ zoi.getWidth()) {
			addScore(1);
			pipe1.setScored(true);
			AssetLoader.coin.play();
		} else if (!pipe2.isScored()
				&& pipe2.getX() + (pipe2.getWidth() / 2) < zoi.getX()
						+ zoi.getWidth()) {
			addScore(1);
			...
			AssetLoader.coin.play();
				...
				&& pipe3.getX() + (pipe3.getWidth() / 2) < zoi.getX()
						+ zoi.getWidth()) {
			addScore(1);
		...
		return (pipe1.collides(zoi) || pipe2.collides(zoi) || pipe3
				.collides(zoi));
	}
